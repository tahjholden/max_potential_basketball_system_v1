...
export default function AddCoachModal({ open, onClose, onCoachAdded }: { open: boolean; onClose: () => void; onCoachAdded?: () => void }) {
  ...
  const isFormValid = coachForm.firstName.trim() && 
    coachForm.lastName.trim() && 
    coachForm.email.trim() && 
    (!canAssignSuperadmin || coachForm.orgId);
  ...
  return (
    <Modal.Add
      ...
      title="Add Coach"
      description="Enter the coach's information below."
      onSubmit={handleAddCoach}
      submitText={loading ? "Adding..." : "Add Coach"}
      loading={loading}
